...
def serve_directory(directory: Path):
    """Serve `directory` on an ephemeral localhost port.

    Chromium refuses ES module imports over file:// (module scripts are
    CORS-checked, and file:// origins are opaque), so the template's import of
    the vendored bundle only resolves over http.
    """
...
def render(
    ...
    try:
        ...
        print("Run: cd ~/.claude/skills/excalidraw-skill/references && uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with serve_directory(template_path.parent) as base_url, sync_playwright() as p:
        try:
            browser = p.chromium.launch(headless=True)
        except Exception as e:
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                print("ERROR: Chromium not installed for Playwright.", file=sys.stderr)
                print("Run: cd ~/.claude/skills/excalidraw-skill/references && uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
        ...
        # Load the template
        page.goto(f"{base_url}/{template_path.name}")

        # Wait for the ES module to load (vendored bundle, served locally)
