...
## Where to Save Diagrams (DO THIS FIRST)

Before creating a diagram, resolve the output directory:

```bash
python3 ~/.claude/skills/excalidraw-skill/references/output_dir.py --get
```

- **Exit 0** — it prints the remembered directory. Save the `.excalidraw` there. Do not ask again.
- **Exit 3** — no default is remembered yet. Ask the user where new diagrams should go,
  using the AskUserQuestion tool, then record their answer:

```bash
python3 ~/.claude/skills/excalidraw-skill/references/output_dir.py --set "<their path>"
```

That writes `~/.config/excalidraw-skill/config.json` and creates the directory, so every
later session skips the question.

Two things override the remembered default, neither of which changes it:
- The user names a path or directory in the request.
- The user asks for the diagram in the current project — then use the repo, not the default.

To point the default somewhere else later: `--set <new path>`. To go back to being asked: `--clear`.
...
```bash
cd ~/.claude/skills/excalidraw-skill/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
